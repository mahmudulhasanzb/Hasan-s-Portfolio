...
export const projects = [
  {
    ...
    problem: 'Traditional library systems are slow, opaque, and geographically limited. Borrowers have no visibility into availability, and the checkout process requires a physical visit, a form, and a librarian.',
    approach: 'Built a full-stack web app with a clean browse/filter UX, a real-time borrowing state machine using MongoDB transactions, and a secure auth system via Better-Auth. Designed the borrow flow to be self-evident — zero learning curve.',
    result: 'A fully functional digital library platform with book browsing, category filtering, user authentication, and a digital borrow/return system with due date tracking.',
    challenges: 'The trickiest part was building an atomic borrow system — preventing two users from borrowing the same last copy simultaneously. I solved it with MongoDB transactions and an optimistic locking pattern on the book status field.',
    futurePlans: 'Add real-time availability notifications via WebSockets, a recommendation engine based on borrow history, and a mobile app using React Native.',
  ...
  },
  {
    ...
    problem: 'AI image tools are powerful, but prompt discovery is broken. Great prompts are scattered across Discord servers, Reddit threads, and private notes — with no organized, searchable, curated home.',
    approach: 'Built a curated prompt library with fast full-text search, tag-based filtering, and a personal collection system. Focused on performance and content quality over feature quantity.',
    result: 'A clean, fast prompt discovery platform with search, tags, user collections, and a curated submission pipeline that keeps quality high.',
    challenges: 'Designing a search system that felt instant even with thousands of prompts was the core challenge. I used MongoDB Atlas Search with debounced queries on the frontend to keep it snappy without hammering the database.',
    futurePlans: 'Implement vector similarity search so users can find prompts by describing what they want in plain English. Also plan a browser extension for saving prompts from anywhere on the web.',
    tech: ['Next.js 15', 'React 19', 'JavaScript', 'Tailwind CSS 4', 'HeroUI', 'Better-Auth', 'MongoDB'],
  ...
  },
  {
    ...
    problem: 'Most developer portfolios look the same — generic templates with no personality. I wanted a portfolio that felt like a product, not a resume dump.',
    approach: 'Designed the entire UI from scratch with a deep-space dark theme, electric-purple accent system, dot-grid backgrounds, and scroll-triggered animations. Every section was planned and iterated.',
    result: 'A fast, accessible, fully responsive portfolio with 10 routes, MDX blog, case study pages, and a contact form — all built with Next.js App Router.',
    challenges: 'Balancing visual richness with performance was tricky. CSS-based glows and dot grids were used instead of canvas/WebGL to keep load times fast. Getting the glassmorphism navbar to look right across browsers required multiple iterations.',
    futurePlans: 'Add a dark/light mode toggle, an interactive terminal easter egg, and connect the contact form to Resend for real email delivery.',
...
export const skillGroups = [
  { category: 'Frontend', color: '#3B82F6', skills: ['HTML5', 'CSS3', 'JavaScript', 'React.js', 'Next.js', 'Tailwind CSS', 'HeroUI', 'Figma'] },
  { category: 'Backend', color: '#10B981', skills: ['Node.js', 'Express.js', 'Better-Auth', 'REST APIs'] },
  { category: 'Database', color: '#8B5CF6', skills: ['MongoDB', 'Mongoose'] },
  { category: 'Tools & DevOps', color: '#F59E0B', skills: ['Git', 'GitHub', 'Docker', 'Vercel'] },
