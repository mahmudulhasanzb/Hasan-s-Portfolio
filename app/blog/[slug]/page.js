...
export default function BlogPostPage({ params }) {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '110px 2rem 6rem' }}>
      ...
      <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'clamp(2rem, 5vw, 3.2rem)', color: 'var(--text)', lineHeight: 1.1, marginBottom: 20 }}>{post.title}</h1>
      <div style={{ display: 'flex', gap: 16, fontSize: 12, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', marginBottom: 40, alignItems: 'center' }}>
        <span>{post.date}</span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><FiClock size={11} />{post.readingTime} min read</span>
      </div>
      <div style={{ height: 1, background: 'linear-gradient(90deg, rgba(124,58,237,0.4), transparent)', marginBottom: '3rem' }} />
